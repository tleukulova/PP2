...
def insert_score(username, current_level):
    conn = connect_db()
    cur = conn.cursor()
    cur.execute("INSERT INTO users (username, current_level) VALUES (%s, %s)", (username, current_level))

def get_top_scores(limit=10):
    conn = connect_db()
    cur = conn.cursor()
    cur.execute("SELECT username, current_level FROM users ORDER BY current_level DESC LIMIT %s", (limit,))
    scores = cur.fetchall()
    cur.close()
    conn.close()
    return scores
...
class Main:
    ...
    def game_over(self):
        ...
        print(f"{name} ate {self.fruits_eaten} apples")
        sys.exit()
...
golden = pygame.image.load('im.png').convert_alpha()
